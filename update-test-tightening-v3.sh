...
async function run() {
  ...
  // ---- Scheduled broadcasts (#33) ----
  console.log('\nScheduled broadcasts (#33)');
  const scheduled = await request('/api/accounting/messages/broadcasts/scheduled');
  result('Scheduled broadcasts list endpoint works', Array.isArray(scheduled.body?.scheduled));
