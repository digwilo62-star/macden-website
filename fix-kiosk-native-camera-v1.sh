...
(function(){
  ...
  // Fully Kiosk Browser's own built-in camera feature -- takes a photo
  // through the app's already-granted Android permission, sidestepping
  // the webpage-level permission prompt entirely. Set FULLY_KIOSK_PASSWORD
  // below to match whatever you configure in Fully Kiosk Browser's own
  // Settings -> Remote Administration -> Remote Admin Password.
  //
  // This only works if:
  //   1. The kiosk is actually running inside Fully Kiosk Browser
  //   2. Remote Administration is enabled in its settings
  //   3. The password below matches what's set there
  // If any of that isn't true, or the request fails for any reason
  // (including a possible HTTPS/mixed-content block, since this page
  // loads over HTTPS but Fully Kiosk's local API is plain HTTP), this
  // silently falls back to the browser's own camera instead -- the
  // approach already tested and confirmed working.
  const FULLY_KIOSK_PASSWORD = 'CHANGE_ME_TO_MATCH_FULLY_KIOSK_SETTINGS';

  async function captureViaFullyKiosk(){
    try {
      const res = await fetch('http://localhost:2323/?cmd=getCamshot&password=' + encodeURIComponent(FULLY_KIOSK_PASSWORD), {
        signal: AbortSignal.timeout(2000) // don't hang the scan if this isn't available
      });
      ...
      return null; // not available -- fall back below, no error shown to the user
    }
  }

  async function captureAttendancePhoto(){
    const fullyKioskPhoto = await captureViaFullyKiosk();
    if (fullyKioskPhoto) return fullyKioskPhoto;

    // Fallback: the browser's own camera access (already tested and
