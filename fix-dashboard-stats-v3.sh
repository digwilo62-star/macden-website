...
# ONE real role-appropriate card. This version normalizes Windows CRLF
# line endings before matching -- that was the actual root cause of the
# last two failures (proven by reproducing your exact warning message
# against real CRLF test content before this fix was built).
set -e
cat > fix-dashboard-stats-v3.js << 'EOF_FIXER_JS'
