#!/usr/bin/env bash
# Combined update to the login page:
# 1. Label fix -- 'Email' -> 'Username or Email' (backend already
#    accepted both, label just never said so)
# 2. Show/hide password toggle -- click the eye icon to reveal the
#    password as plain text, click again to hide it
# Tested against a full reconstruction including the MFA second-step
# form to confirm the toggle doesn't interfere with either form's
# submit logic.
set -e
cat > fix-login-label-and-eye.js << 'EOF_FIXER_JS'
...
node fix-login-label-and-eye.js
echo "Done. Hard-refresh (Ctrl+F5) -- HTML/CSS/JS only, no server restart needed."