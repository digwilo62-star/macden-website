...
// 3. Add CSS for the new file-manager-style attachment chip, right before </style>
const styleCloseAnchor = `</style>`;
const newStyles = `  .email-attachment{ display:inline-flex; align-items:center; gap:9px; text-decoration:none; padding:8px 12px 8px 8px; border:1px solid var(--border); border-radius:10px; background:var(--surface); margin-top:8px; transition:border-color 0.12s ease; }
...
if (!content.includes(styleCloseAnchor)) {
  console.error('ERROR: could not find the closing </style> tag in inbox.html.');
  process.exit(1);
}
// Only replace the FIRST occurrence (the main stylesheet close), not any later ones
content = content.replace(styleCloseAnchor, newStyles);
