...
<div class="card front">
  ...
  <div class="microring" id="microring"></div>
  <div class="header">
...
(function(){
  ...
  async function downloadCardAsPDF(fullName, staffId, photoUrl){
    ...
    try {
      ...
      // The microring watermark has the exact same background-image
      // blurriness problem as the photo did -- same fix applies.
      const microring = document.getElementById('microring');
      if (microring) {
        const ringImg = await loadImage('assets/microring.png');
        const ringCanvas = document.createElement('canvas');
        const ringScale = 6;
        ringCanvas.width = microring.offsetWidth * ringScale;
        ringCanvas.height = microring.offsetHeight * ringScale;
        ringCanvas.style.width = '100%';
        ringCanvas.style.height = '100%';
        ringCanvas.style.display = 'block';
        const ringCtx = ringCanvas.getContext('2d');
        ringCtx.drawImage(ringImg, 0, 0, ringCanvas.width, ringCanvas.height);

        microring.style.backgroundImage = 'none';
        microring.appendChild(ringCanvas);
        tempCanvases.push({ el: microring, canvas: ringCanvas, restoreBg: "url('assets/microring.png')" });
      }
