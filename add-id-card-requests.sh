#!/bin/bash
# add-id-card-requests.sh
#
# Adds the staff ID card request/approval flow:
#   - staff submit a request (backend only -- see note below for the
#     settings.html button, which is NOT auto-inserted)
...
#
# Does NOT touch server/server.js, settings.html, or manage-staff.html.
# Route mounting and the two frontend snippets are handled separately,
# same caution as the last patch -- not editing files sight-unseen.
...
<div class="card front">
  ...
  <div class="seal"><img src="/portal/assets/logo-seal-mono.png" alt=""></div>
...
(function(){
  ...
  fetch('/api/id-card/card/' + encodeURIComponent(requestId))
...
echo "   SQL editor."
echo ""
echo "2. Add portal/assets/logo-seal-mono.png -- the watermark seal image"
echo "   referenced by id-card-view.html. This was generated earlier this"
echo "   session and needs to be added to your repo's portal/assets/"
echo "   folder manually (separate download)."
echo ""
echo "3. Add a 'Request ID Card' button to portal/settings.html -- see the"
echo "   separate settings-button-snippet.html for the exact block to paste"
echo "   in. Not auto-inserted since I have not seen that file's contents."
echo ""
echo "4. Add a link to portal/id-card-requests.html somewhere in"
echo "   portal/manage-staff.html (or portal/dashboard.html) so admins can"
echo "   actually find the approval queue. Same reason -- not auto-inserted."
echo ""
echo "5. *** VERIFY the auth assumption in server/routes/idCardRequests.js ***"
echo "   Look for getSessionStaffId() and isAdmin() near the top of the file."
echo "   These assume req.session.staffId and req.session.role === 'admin'."
echo "   If your actual session shape differs, fix those two functions --"
echo "   they are the only places this assumption is used."
echo ""
