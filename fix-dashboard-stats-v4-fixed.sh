#!/usr/bin/env bash
# v4-fixed: same as v4, but the regex now tolerates a space before
# '(coming soon)' on the Tasks Assigned label. Root cause found via
# piece-by-piece regex testing against your real file: your terminal
# was eating that one space when displaying pastes (same class of
# artifact as 'divclass'/'newDate' from earlier), so I'd hard-coded the
# wrong (space-less) version into the pattern.
set -e
...
const statGridRegex = /<div class="stat-grid">\s*<div class="stat-card">\s*<div class="num" id="statUnread">[^<]*<\/div>\s*<div class="lbl">Unread Messages<\/div>\s*<\/div>\s*<div class="stat-card">\s*<div class="num">[^<]*<\/div>\s*<div class="lbl">Pending Requests \(coming soon\)<\/div>\s*<\/div>\s*<div class="stat-card">\s*<div class="num">[^<]*<\/div>\s*<div class="lbl">Tasks Assigned\s*\(coming soon\)<\/div>\s*<\/div>\s*<div class="stat-card">\s*<div class="num">[^<]*<\/div>\s*<div class="lbl">Approvals \(coming soon\)<\/div>\s*<\/div>\s*<\/div>/;
