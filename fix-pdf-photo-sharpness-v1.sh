  ...
  async function downloadCardAsPDF(fullName, staffId, photoUrl){
    ...
    try {
      ...
      if (photoUrl) {
        ...
        tempCanvases.push({ el: mainPhoto, canvas: mainCanvas });
        ...
        tempCanvases.push({ el: ghostPhoto, canvas: ghostCanvas });
      }
      ...
      tempCanvases.forEach(({ el, canvas }) => el.removeChild(canvas));
      btn.disabled = false;
    }
  }
})();
