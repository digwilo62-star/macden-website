#!/usr/bin/env bash
# Adds the missing 'Pending Staff Approvals' panel to the Dashboard,
# admin-only -- this existed in the backend all along but never got
# added back to the UI when the dashboard was rebuilt.
# Caught and fixed a real bug during testing: an early version put the
# panel-load call inside the wrong function due to anchor ambiguity --
# fixed by reordering the patch steps, verified against your real
# init() structure before being sent.
set -e
cat > fix-pending-staff.js << 'EOF_FIXER_JS'
