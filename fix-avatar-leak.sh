bash fix-avatar-leak.sh#!/usr/bin/env bash
# Fixes the topbar avatar switching to whoever's profile you last
# viewed in Directory. Root cause: the avatar update was triggered by
# the SHAPE of any API response, not by which endpoint was called --
# fragile, and exactly the kind of thing that leaks someone else's
# photo into your own avatar. Fixed by checking the request path
# instead. Proven with a real behavioral test (not just a syntax
# check) simulating the exact leak scenario before this was sent.
set -e
cat > fix-avatar-leak.js << 'EOF_FIXER_JS'
