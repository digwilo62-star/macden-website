...
<style>
  ...
  .fs-header-row{ display:grid; min-width:750px; grid-template-columns: minmax(140px,1fr) minmax(100px,140px) minmax(100px,140px) minmax(110px,140px) minmax(70px,90px) minmax(260px,320px); gap:12px; padding:12px 18px; font-size:10.5px; font-weight:700; text-transform:uppercase; letter-spacing:0.04em; color:var(--text-muted); border-bottom:1px solid var(--border); }
  .fs-row{ display:grid; min-width:750px; grid-template-columns: minmax(140px,1fr) minmax(100px,140px) minmax(100px,140px) minmax(110px,140px) minmax(70px,90px) minmax(260px,320px); gap:12px; align-items:center; padding:14px 18px; border-bottom:1px solid var(--border); font-size:12.5px; }
  ...
  .fs-action-btn{ border:1px solid var(--border); background:var(--surface); border-radius:var(--radius-sm); padding:5px 9px; font-size:10.5px; font-weight:600; cursor:pointer; font-family:var(--font-body); color:var(--text-primary); white-space:nowrap; transition:all 0.12s ease; }
  ...
  .fs-actions-row{ display:flex; gap:6px; flex-wrap:nowrap; }
  .fs-row:hover{ background:var(--surface-raised, rgba(0,0,0,0.015)); }
...
<body>
  ...
  <div class="modal-backdrop" id="fieldStaffModalBackdrop">
    <div class="modal">
      ...
      <div id="fsDeleteSection" style="display:none; margin-top:18px; padding-top:16px; border-top:1px solid var(--border);">
        <button type="button" class="btn btn-ghost" id="fsDeleteBtn" style="width:auto; padding:8px 14px; font-size:12.5px; color:var(--error); border-color:var(--error);">Delete Field Worker</button>
        <p style="font-size:11px; color:var(--text-muted); margin-top:6px;">Permanent -- their record and QR code stop working immediately. Use Deactivate instead if you just want to disable their card temporarily.</p>
      </div>
    </div>
  </div>
  ...
  <script>
      ...
      try {
        ...
        rows.innerHTML = data.fieldStaff.map(p => {
          ...
          const avatar = p.photo_url
            ? '<img src="' + p.photo_url + '" style="width:28px; height:28px; border-radius:50%; object-fit:cover; vertical-align:middle; margin-right:10px; border:1px solid var(--border);">'
            : '<span style="display:inline-block; width:28px; height:28px; border-radius:50%; background:var(--gold-dim); vertical-align:middle; margin-right:10px;"></span>';
    ...
    function openAddModal() {
      ...
      const inventoryDept = departmentsList.find(d => (d.name || '').toLowerCase() === 'inventory');
      document.getElementById('fsDepartmentId').value = inventoryDept ? inventoryDept.id : '';
      ...
      document.getElementById('fsDeleteSection').style.display = 'none';
    ...
    async function editFS(id) {
      try {
        ...
        editingId = id;
        editingName = p.full_name || 'this person';
        ...
        document.getElementById('fsDeleteSection').style.display = 'block';
    ...
    document.getElementById('fsDeleteBtn').addEventListener('click', async () => {
      if (!editingId) return;
      if (!confirm('Permanently delete ' + editingName + '? This cannot be undone -- their record and QR code will stop working immediately.')) return;

      try {
        const res = await fetch('/api/field-staff/' + editingId, { method: 'DELETE', credentials: 'include' });
        const data = await res.json();
