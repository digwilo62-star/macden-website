...
# admin-only. Reuses the existing DELETE /admin/staff/:id route (built
# earlier for Manage Staff) -- no new backend needed. Reversible via
# Manage Staff's Reactivate button. Tested against your real file
# structure and confirmed correct placement inside openProfile() before
# being sent.
set -e
cat > fix-directory-deactivate.js << 'EOF_FIXER_JS'
// Adds a "Deactivate" button to the staff profile modal in Directory,
