#!/usr/bin/env bash
# Fixes the ENOENT crash properly this time -- corrects BOTH the
# earlier broken /portal route AND the static file mount itself, which
# also still referenced the old 'accounting' folder name (a pre-
# existing inconsistency, not something the earlier fix introduced).
# Both now consistently point at 'portal', the folder confirmed to
# actually exist on disk. Proven with a real running server: login
# page (both /portal and /portal/) AND static file serving through the
# corrected mount all tested and confirmed 200 OK with real content.
set -e
cat > fix-portal-folder-consistency.js << 'EOF_FIXER_JS'
...
node fix-portal-folder-consistency.js
echo "Done. Push, wait for Render, then test macden.com.ng/portal/ again."