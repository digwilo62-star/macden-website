#!/usr/bin/env bash
# TIGHTENING BATCH 4: field-level encryption for NIN/address (a real
# security gap flagged when these fields were first added), an Org Chart
# view built from the reports_to data already being captured, and a real
# in-app Help guide linked from every page.
# NO SQL MIGRATION NEEDED - nin/address columns already exist, encrypted
# values just store as longer text in the same columns.
# YOU MUST add a new ENCRYPTION_KEY to .env (see instructions below).
# Run this from the ROOT of your macden-website repo, in Git Bash.
set -e

mkdir -p server/utils server/routes accounting
...
# Used to encrypt sensitive fields (NIN, address) at the application level.
# Generate one with: node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
ENCRYPTION_KEY=replace-with-a-generated-64-character-hex-string
...
cat > server/utils/encryption.js << 'EOF_SERVER_UTILS_ENCRYPTION_JS'
...
EOF_SERVER_UTILS_ENCRYPTION_JS
...
echo "Tightening batch 4 complete: 15 of 40 items now done."
echo "IMPORTANT: add ENCRYPTION_KEY to .env before onboarding any new staff."
