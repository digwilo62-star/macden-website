#!/usr/bin/env bash
# Adds show/hide eye-icon toggles to Settings' 3 password fields
# (Current, New, Confirm) -- same pattern as the login page. One
# shared toggle function handles all three via data-target, instead of
# duplicating the logic. Tested against real content, confirmed the
# existing password-change logic (reading .value from each input)
# still works correctly after wrapping each field in a positioning div.
set -e
cat > fix-settings-password-eyes.js << 'EOF_FIXER_JS'
