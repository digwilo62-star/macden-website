...
# fresh random password (overwriting whatever they set during
# registration), forces a change on first login, and emails them the
# real login details -- reusing the exact same email function already
# built for HR-onboarded accounts, no new template needed.
# This is server/routes/admin.js -- unaffected by the accounting/portal
# rename, since backend routes were never touched by that.
set -e
cat > fix-approve-generates-password.js << 'EOF_FIXER_JS'
// When admin approves a self-registered account, this now generates a
