...
# fix-photo-resize.sh
#
# Adds automatic resize + compression to the staff photo upload route
# (server/routes/settings.js). Every uploaded photo -- regardless of its
# original size or format -- gets:
#   - auto-rotated based on EXIF data (fixes sideways phone photos)
#   - capped to 1200x1200 max (preserving aspect ratio, never upscaled)
#   - re-encoded as JPEG at quality 82 (reliably lands in the 80-250KB
#     range for typical photos, instead of whatever the original was)
#
# This runs entirely server-side, before the file reaches Supabase storage.

set -e

if grep -q "sharp(req.file.buffer)" server/routes/settings.js; then
...
cat > .tmp-patch-photo.js << 'NODE_EOF'
const fs = require('fs');
...
node .tmp-patch-photo.js
rm .tmp-patch-photo.js
