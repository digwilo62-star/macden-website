#!/usr/bin/env bash
# Adds the 2 diagnostic lines that failed to add last time (fragile
# multi-line match). This version targets a single distinctive line
# instead -- confirmed working against multiple formatting variants.
set -e
cat > add-email-step-logging.js << 'EOF_FIXER_JS'
...
node add-email-step-logging.js
echo "Done. Push, wait for Render, then click Approve while watching Logs again."