...
#!/usr/bin/env bash
# BACKEND ITEM #33: Scheduled broadcasts. Adds an optional scheduledAt
# field to POST /messages/broadcast; a cron job checks every minute for
# due scheduled broadcasts and actually sends them (creates read rows,
# fires notification emails). HONEST LIMITATION: Render's free tier
# sleeps when idle - a scheduled send can land a few minutes late
# rather than at the exact second, since it only fires once the app is
# actually awake. Fine for 'send this tomorrow morning', not for
# anything needing second-level precision.
...
echo "Scheduled broadcasts backend complete (#33). This closes out ALL backend-buildable items. 23 of 40 total done."
echo "Run npm install in server/ to get the new node-cron dependency."