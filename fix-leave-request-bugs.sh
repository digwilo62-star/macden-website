...
#    relied entirely on an assumed database default. This is the
#    likely actual cause of requests not showing up in Pending
#    Approvals for ANY admin, not specific to one account.
# 2. Approve and Reject both called newDate() instead of new Date() --
#    CONFIRMED with a direct test to throw a real error. This means
#    approving/rejecting leave requests has likely never actually
#    worked -- every attempt would fail before the database update
#    even runs.
set -e
cat > fix-leave-request-bugs.js << 'EOF_FIXER_JS'
// Fixes two real bugs in leave.js:
// 1. Submitting a request never explicitly set status: 'pending',
