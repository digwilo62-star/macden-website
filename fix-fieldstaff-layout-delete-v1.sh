...
<style>
  ...
  .fs-row{ display:grid; min-width:750px; grid-template-columns: minmax(140px,1fr) minmax(100px,140px) minmax(100px,140px) minmax(110px,140px) minmax(70px,90px) minmax(260px,320px); gap:12px; align-items:center; padding:12px 18px; border-bottom:1px solid var(--border); font-size:12.5px; }
  ...
  .fs-action-btn{ border:1px solid var(--border); background:var(--surface); border-radius:var(--radius-sm); padding:5px 9px; font-size:10.5px; font-weight:600; cursor:pointer; font-family:var(--font-body); color:var(--text-primary); white-space:nowrap; }
  .fs-action-btn:hover{ border-color:var(--primary); color:var(--primary); }
  .fs-action-btn.danger{ color:var(--error); border-color:var(--error); }
  .fs-action-btn.danger:hover{ background:var(--error-dim); }
